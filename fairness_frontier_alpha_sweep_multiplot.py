...
nAlpha = 20
weights = np.linspace(0,1,nAlpha) #should all be >= 1
weightstr = f"linspace(0,1,{nAlpha})"
...
def plot_tradeoff(best,frontiers,saving=False):
	...
	ax = fig.add_subplot(gs[0],label=SWB_prob.__name__,frame_on=False)#,
	# ax = fig.add_subplot(111,label=fn.__name__,frame_on=False)
	ax1 = fig.add_subplot(gs[1],label="alpha_plot",frame_on=True)#,
	...
	frontier = frontiers[SWB_prob]

	axes[SWB_prob] = ax
	line_color = color_map[SWB_prob.weightstr]
	def plot_frontier():
		x_raw = frontier.fstarvals_type0()
		y_raw = frontier.fstarvals()
		print(f"Plotting data for {SWB_prob.__name__}")
		linename = f"$\\sum$ {SWB_prob.basebasename} Efficient Frontier"
	...
	print(f"Scattering for {SWB_prob.__name__}")
	cmap = plt.cm.get_cmap('cividis')
	norm = Normalize(vmin=min(weights), vmax=max(weights))
	sm = ScalarMappable(norm=norm, cmap=cmap)
	def scatter_objectives():
		scat_x_vals = [instance.fstars_type0[SWB_prob] for other_fn,instance in best.items()]
		scat_y_vals = [instance.fstars[SWB_prob] for other_fn,instance in best.items()]
	...
	def plot_alpha_ranges():
		instances = tuple(set(best.values()))
		insts_at_ranges = {(min(ll:=[the_fn.weight for the_fn,the_instance in best.items() if the_instance == inst]),max(ll)): inst for inst in instances}
		ranges = sorted(insts_at_ranges.keys(),key=lambda tup:tup[0])
		xvals = []
		yvals_type0 = []
		yvals_pop = []
		for (lb,ub) in ranges:
	...
	scat[SWB_prob] = [matplotlib.lines.Line2D([], [],markersize=10,color=sm.to_rgba(other_fn.weight), marker=other_fn.mpl_marker,alpha=0.8,linestyle='None') for i,(other_fn,instance) in enumerate(best.items())]
	...
	scat_dict = {}
	for SWB_prob,art_list in scat.items():
	...
	print(scat_dict)
	ax.legend(legend_dict.values(),legend_dict.keys(),loc='lower left',handler_map={tuple: HandlerTuple(ndivide=None)},labelspacing=0.9,handlelength=3.5) #,handletextpad=1.0,loc="upper right"bbox_to_anchor=(0.5, 0), ncol=len(legend_dict),fontsize='small'
